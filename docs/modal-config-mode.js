...
(function() {
  ...
  const ModalConfigMode = {
    ...
    /**
     * Show highlight cutout during tour playback (responsive with clamp bounds)
     */
    showHighlight(highlightConfig) {
      // Remove existing highlight
      this.hideHighlight();

      // Apply clamp() bounds for viewport units to ensure min/max sizing
      const clampValue = (value, minPx, maxPercent) => {
        if (typeof value === 'string') {
          if (value.endsWith('vw')) {
            const vw = parseFloat(value);
            return `clamp(${minPx}px, ${vw}vw, ${maxPercent}vw)`;
          }
          if (value.endsWith('vh')) {
            const vh = parseFloat(value);
            return `clamp(${minPx}px, ${vh}vh, ${maxPercent}vh)`;
          }
        }
        return value; // Pass through px or % values unchanged
      };

      const clampedWidth = clampValue(highlightConfig.width, 280, 95);
      const clampedHeight = clampValue(highlightConfig.height, 150, 90);
      ...
      console.log('[ModalConfigMode] Highlight shown (responsive):', {
        width: clampedWidth,
        height: clampedHeight,
        top: highlightConfig.top,
        left: highlightConfig.left
